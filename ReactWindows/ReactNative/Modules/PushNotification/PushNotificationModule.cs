...
namespace ReactNative.Modules.PushNotification
{
    ...
    class PushNotificationModule : ReactContextNativeModuleBase
    {
        ...
        [ReactMethod]
        public void requestPermissions(string appID)
        {
            Task.Run(async () =>
            {
                try
                {
                    if (appID != null)
                    {
                        _channel = await PushNotificationChannelManager.CreatePushNotificationChannelForApplicationAsync(appID);
                    }
                    else
                    {
                ...
                if (_channel != null)
                {
                    ...
                    EventEmitter.emit("remoteNotificationsRegistered", new JArray
                    {
                        _channel.Uri,
                        _channel.ExpirationTime,
                    });
                }
            });
        }
